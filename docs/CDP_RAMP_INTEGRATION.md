...
- **Why 3.** No other cash-out retry constant exists to match, so this is the conservative pick. A player whose first send fails for an honest reason still gets two more tries, and a hostile row costs the house at most three failed landings, about 330,000 lamports (0.00033 SOL) at the default cosign fee ceiling of roughly 110,000 lamports per landing (`COSIGN_FEE_MARGIN`). The bound is per row, so starting a fresh cash-out resets it; each fresh cash-out costs the player a new Coinbase sell session and stays under the `cdp_offramp_send/user` throttle.
