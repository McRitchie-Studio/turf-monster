...
3. `POST /cdp/offramp/cosign_send` — server rebuilds the expected transfer from its own row via `Solana::Vault#usdc_transfer_expectation` (wallet, destination, amount), then hands the returned bytes and that expectation to `#cosign_usdc_transfer`, which judges the wire before it fills the admin slot, then simulates the cosigned wire via `#preflight_cosigned_wire!`, and hands back the fully-signed wire only when the simulation passes. The judgment now lives inside the cosign itself, not as a separate step a caller could skip: on reject nothing is signed or returned, no claim is taken, and the forensic reason is logged server-side but never returned to the client.
...
**Why the guard is mandatory.** The admin is the fee payer, so filling its signature slot is the house agreeing to pay for whatever bytes the client hands back. The guard is `Solana::Cosign::Expectation`, the same one the entry and create_contest cosigns judge their wires against: the signer set must be exactly the admin plus the cash-out wallet, and every instruction — once ComputeBudget and an admitted Lighthouse assertion are set aside — must equal, byte for byte, the one SPL Token Transfer the server built: the exact expected amount, from the wallet's own USDC ATA, to the resolved destination, under the wallet's own authority. Any other instruction, System included, is refused by that exact-match rule rather than by a named case. The Lighthouse check lives in the same `Expectation`: it admits assertion variants 2–17 and refuses `MemoryWrite` (0), `MemoryClose` (1), empty data, and any unknown variant. The memory refusal is what protects the house here: `MemoryWrite` makes the signer it names as payer fund a memory account, and the house signs this wire as fee payer, so without it a crafted cash-out could lock house SOL. The check reads the first data byte rather than whether an instruction names the fee payer, because the real Phantom mainnet wires the house has cosigned carry `AssertAccountInfoMulti` (6) and `AssertTokenAccountMulti` (10), and each has a variant-6 assertion on the fee payer's own post-state.

**Why the signer count is pinned.** Solana charges the fee payer 5,000 lamports for every signature the message declares, and charges it when a landing fails too. The priority half of the fee was already capped; the base half was not, so until 2026-09-17 a player could pad the cash-out wire with signer slots they filled themselves. Ten signers fit Solana's 1,232-byte packet beside a valid transfer, which raised the house's cost per failed landing from 110,000 to 150,000 lamports. `Solana::Cosign::Expectation#verify!` now refuses any wire whose signer set is not exactly the admin plus the cash-out wallet (`signer_set_mismatch`) — an extra signer or a missing one, either way. The entry and create_contest cosigns run the same rule. Lighthouse adds instructions, never signers, so Phantom-protected wires still pass: all five real mainnet wires the house has cosigned declare exactly two signers, the house then the player, pinned byte for byte in `test/fixtures/files/phantom_mainnet_cosigned_wires.json` (pin-cashout-cosign-signer-count).

**Why the pre-flight is mandatory.** The client broadcasts the cosigned wire with `skipPreflight: true`, so no node checks it after the house signs, and a wire that fails on chain still charges its fee payer — the house. The entry and contest-create cosigns simulate before they broadcast too, inside `Solana::Cosign::Completer#complete`; `Solana::Vault#preflight_cosigned_wire!` is the same check for the one cosign path that does not broadcast itself. It uses the entry path's settings (`sig_verify: false`, `replace_recent_blockhash: true`): the house pays only for a transaction that executes, and a wire with a bad signature or a dead blockhash never executes, so the program verdict is the one that protects the fee payer. It fails closed. A failed simulation, one that could not be run, one that answered nothing, and one whose answer has no `err` field all raise `PreflightRejected`, and the program error and logs go to the `ErrorLog`. The refusals that gave **no verdict** are its subclass `PreflightUnavailable`, so the player reads different copy for each: an outage gets "Solana is busy right now — please try again in a moment." (502), and a wire the program refused gets "Check that your wallet still holds the USDC, then start the cash-out again." (422). A missing `err` is refused because `sim["err"]` reads a missing key exactly like a clean `"err": null`. No conforming node omits it: agave's `RpcSimulateTransactionResult` serializes `err` with no `skip_serializing_if`, and read-only `simulateTransaction` calls to Helius and the public mainnet RPC on 2026-09-17 returned `"err": null` for a clean simulation and `"err": "AccountNotFound"` for a failing one. So the refusal should never fire on a real answer; it guards against a proxy, a truncated body, or a future shape change. The simulate-and-read-err logic is one private helper, `Solana::Vault#simulate_wire!`, shared with this pre-flight and the multisig settle broadcast (`#simulate_and_broadcast`) — the entry and contest-create cosigns simulate through `Solana::Cosign::Completer` instead and no longer reach it; each caller keeps its own error types and empty-answer handling, pinned in `test/services/solana/vault_simulate_callers_test.rb`.
