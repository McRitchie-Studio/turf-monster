...
**Preconditions:** at least one contest in status `open`/`settled` exists, or `Contest.featured` returns nil and root falls back to `/contests` (`app/models/contest.rb:200-205`; `locked` is no longer a status — it is a derived time-gate). `PAYMENT_PROVIDER=stripe` plus Stripe keys set (`Rails.application.config.x.stripe_enabled`, checked in `TokensController#stripe_checkout` at `app/controllers/tokens_controller.rb:29-31`). A `SeasonConfig` row with a non-zero `current_season_id`, enforced on the entry path by `ContestsController#ensure_onchain_season_ready!` (`app/controllers/contests_controller.rb:2483-2487`). The chosen contest must be on-chain — the token branch of `ContestsController#resolve_web2_entry_funding!` is what consumes the `EntryTokenAccount` (`:1934-1946`).
...
10. **Hold-to-Confirm fires `POST /contests/:id/enter`** — `ContestsController#enter` (`app/controllers/contests_controller.rb:739-961`).
    ...
    - `enter` loads the cart entry at `:776`, and the web2 / managed-wallet funding branch runs inside its `@contest.with_lock` (`:893`) through `ContestsController#resolve_web2_entry_funding!` (`:1918-1990`, documented at `:1898-1909`):
      - `current_user.next_unconsumed_entry_token_for(address)` reads the token on-chain (`:1934`, definition `User#next_unconsumed_entry_token_for` at `app/models/user.rb:729-736`). With no token and no USDC the branch raises `"No entry tokens. Buy at /tokens/buy"` (`app/controllers/contests_controller.rb:1988`).
      - `Solana::Vault#enter_contest_with_token` is the atomic Anchor instruction — it creates the entry PDA, consumes the token, and awards seeds (`:1938-1941`, definition `app/services/solana/vault.rb:1446`). The managed wallet's keypair, decrypted from the DB by `User#solana_keypair` (`app/models/user.rb:552-555`), signs it.
    - `entry.confirm!(tx_signature:, onchain_entry_id:)` runs from `ContestsController#finalize_managed_entry!` (`app/controllers/contests_controller.rb:2090`). `Entry#confirm!` (`app/models/entry.rb:170-208`) re-runs `assert_enterable!` under the user row lock (`:176`), refuses a paid entry with no payment proof (`:185-187`), writes the `entry_fee` `TransactionLog` debit (`:189-191`), and flips `entries.status` → `active` (`:192`). The 6-selection count, lock time, and duplicate-combo checks all live in `Entry#assert_enterable!` (`:125-159`).
...
- **Wallet has no unconsumed entry token at `enter`** — `ContestsController#resolve_web2_entry_funding!` raises `"No entry tokens. Buy at /tokens/buy"` (`app/controllers/contests_controller.rb:1988`); the client surfaces a CTA.
