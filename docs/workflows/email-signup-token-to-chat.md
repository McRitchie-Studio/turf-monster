...
**Preconditions:** at least one contest in status `open`/`settled` exists, or `Contest.featured` returns nil and root falls back to `/contests` (`app/models/contest.rb:200-205`; `locked` is no longer a status — it is a derived time-gate). `PAYMENT_PROVIDER=stripe` plus Stripe keys set (`Rails.application.config.x.stripe_enabled`, checked in `TokensController#stripe_checkout` at `app/controllers/tokens_controller.rb:29-31`). A `SeasonConfig` row with a non-zero `current_season_id`, enforced on the entry path by `ContestsController#ensure_onchain_season_ready!` (`app/controllers/contests_controller.rb:2509-2513`). The chosen contest must be on-chain — the token branch of `ContestsController#resolve_web2_entry_funding!` is what consumes the `EntryTokenAccount` (`:1960-1972`).

## Sequence

1. **Visitor lands on `/`** — `root "contests#world_cup"` (`config/routes.rb:57`) → `ContestsController#world_cup` (`app/controllers/contests_controller.rb:640-644`).
   ...
   - `world_cup` then 302s to `contest_path(@contest)` (`app/controllers/contests_controller.rb:643`).

2. **Show page renders for a logged-out visitor** — `ContestsController#show` (`app/controllers/contests_controller.rb:646-687`) → `app/views/contests/show.html.erb`.
...
8. **Back to root → main contest** — the user clicks the navbar "Turf Monster" home link → `GET /` → step 1 repeats → 302 to `contest_path(@contest)`.
   - Same `ContestsController#world_cup` action, same `Contest.featured` chain (`app/controllers/contests_controller.rb:641`).
...
9. **Build a 6-pick lineup** — each tap on a matchup tile POSTs to `ContestsController#toggle_selection` (`app/controllers/contests_controller.rb:1504-1525`).
   - It rejects the tap unless the contest is `open?` (`:1505-1507`).
   - `find_or_create_by!(user: current_user, status: :cart)` creates the cart `Entry` on the first toggle (`:1512`).
   - `entry.toggle_selection!(matchup)` (`:1515`) enforces the cap of `contest.picks_required` — 6 — inside `Entry#toggle_selection!` (`app/models/entry.rb:43-68`), which replaces the oldest pick once the cap is reached (`:53-59`).
   - The body is wrapped in `rescue_and_log(target: entry, parent: @contest)` (`app/controllers/contests_controller.rb:1514`).

10. **Hold-to-Confirm fires `POST /contests/:id/enter`** — `ContestsController#enter` (`app/controllers/contests_controller.rb:739-961`).
    - Gated by the class-body `before_action :require_geo_allowed` (`:16`) and `before_action :require_unfrozen_account` (`:18`).
    - `enter` loads the cart entry at `:776`, and the web2 / managed-wallet funding branch runs inside its `@contest.with_lock` (`:893`) through `ContestsController#resolve_web2_entry_funding!` (`:1944-2016`, documented at `:1924-1935`):
      - `current_user.next_unconsumed_entry_token_for(address)` reads the token on-chain (`:1960`, definition `User#next_unconsumed_entry_token_for` at `app/models/user.rb:729-736`). With no token and no USDC the branch raises `"No entry tokens. Buy at /tokens/buy"` (`app/controllers/contests_controller.rb:2014`).
      - `Solana::Vault#enter_contest_with_token` is the atomic Anchor instruction — it creates the entry PDA, consumes the token, and awards seeds (`:1964-1967`, definition `app/services/solana/vault.rb:2251`). The managed wallet's keypair, decrypted from the DB by `User#solana_keypair` (`app/models/user.rb:552-555`), signs it.
    - `entry.confirm!(tx_signature:, onchain_entry_id:)` runs from `ContestsController#finalize_managed_entry!` (`app/controllers/contests_controller.rb:2116`). `Entry#confirm!` (`app/models/entry.rb:170-208`) re-runs `assert_enterable!` under the user row lock (`:176`), refuses a paid entry with no payment proof (`:185-187`), writes the `entry_fee` `TransactionLog` debit (`:189-191`), and flips `entries.status` → `active` (`:192`). The 6-selection count, lock time, and duplicate-combo checks all live in `Entry#assert_enterable!` (`:125-159`).
    - The JSON response carries `redirect: contest_path(@contest)` (`app/controllers/contests_controller.rb:948`).
...
- `entries` (insert + update) — created `status: :cart` by `ContestsController#toggle_selection` (`app/controllers/contests_controller.rb:1512`), flipped to `:active` with `onchain_tx_signature` + `onchain_entry_id` by `Entry#confirm!` (`app/models/entry.rb:192`).
...
- **on-chain**: `UserAccount` PDA (created by `CreateOnchainUserAccountJob` post-signup); one `EntryTokenAccount` PDA per mint through `Solana::Vault#mint_entry_token`, which refuses locally once the v0.26 per-window mint cap is reached rather than broadcasting a doomed transaction (`app/services/solana/vault.rb:2654`, `source: :stripe`); the entry PDA created and the token PDA consumed atomically by `Solana::Vault#enter_contest_with_token` (`:2251`, turf-vault v0.12.0+).
...
- **`SeasonConfig.current_season_id == 0` at entry time** — `ContestsController#onchain_season_error` returns `"No active season configured. Set one at /admin/seasons before creating on-chain contests."` (`app/controllers/contests_controller.rb:2515-2517`) and `#ensure_onchain_season_ready!` raises it (`:2509-2513`). The user sees a toast; the operator fix is `/admin/seasons` → set current.
- **No open contest** — `ContestsController#world_cup` returns `redirect_to contests_path` (`app/controllers/contests_controller.rb:642`); the user lands on the contest index instead of a show page.
...
- **Contest is full at `enter`** — `Entry#assert_enterable!` raises `"Contest is full"` (`app/models/entry.rb:146-147`), run as the pre-flight inside `ContestsController#enter`'s `with_lock` (`app/controllers/contests_controller.rb:896`) so the token is never consumed. The JSON 422 surfaces via the board toast.
- **Wallet has no unconsumed entry token at `enter`** — `ContestsController#resolve_web2_entry_funding!` raises `"No entry tokens. Buy at /tokens/buy"` (`app/controllers/contests_controller.rb:2014`); the client surfaces a CTA.
