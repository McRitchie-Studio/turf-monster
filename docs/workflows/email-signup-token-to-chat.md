...
10. **Hold-to-Confirm fires `POST /contests/:id/enter`** — `ContestsController#enter` (`app/controllers/contests_controller.rb:739-961`).
    ...
    - `enter` loads the cart entry at `:776`, and the web2 / managed-wallet funding branch runs inside its `@contest.with_lock` (`:893`) through `ContestsController#resolve_web2_entry_funding!` (`:1944-2016`, documented at `:1924-1935`):
      ...
      - `Solana::Vault#enter_contest_with_token` is the atomic Anchor instruction — it creates the entry PDA, consumes the token, and awards seeds (`:1964-1967`, definition `app/services/solana/vault.rb:2046`). The managed wallet's keypair, decrypted from the DB by `User#solana_keypair` (`app/models/user.rb:552-555`), signs it.
...
- **on-chain**: `UserAccount` PDA (created by `CreateOnchainUserAccountJob` post-signup); one `EntryTokenAccount` PDA per mint through `Solana::Vault#mint_entry_token`, which refuses locally once the v0.26 per-window mint cap is reached rather than broadcasting a doomed transaction (`app/services/solana/vault.rb:2449`, `source: :stripe`); the entry PDA created and the token PDA consumed atomically by `Solana::Vault#enter_contest_with_token` (`:2046`, turf-vault v0.12.0+).
