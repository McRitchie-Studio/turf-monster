...
10. **Hold-to-Confirm fires `POST /contests/:id/enter`** — `ContestsController#enter` (`app/controllers/contests_controller.rb:716-938`).
    ...
    - `enter` loads the cart entry at `:753`, and the web2 / managed-wallet funding branch runs inside its `@contest.with_lock` (`:870`) through `ContestsController#resolve_web2_entry_funding!` (`:1878-1950`, documented at `:1858-1869`):
      ...
      - `Solana::Vault#enter_contest_with_token` is the atomic Anchor instruction — it creates the entry PDA, consumes the token, and awards seeds (`:1898-1901`, definition `app/services/solana/vault.rb:1404`). The managed wallet's keypair, decrypted from the DB by `User#solana_keypair` (`app/models/user.rb:552-555`), signs it.
...
- **on-chain**: `UserAccount` PDA (created by `CreateOnchainUserAccountJob` post-signup); one `EntryTokenAccount` PDA per mint through `Solana::Vault#mint_entry_token` (`app/services/solana/vault.rb:1770`, `source: :stripe`); the entry PDA created and the token PDA consumed atomically by `Solana::Vault#enter_contest_with_token` (`:1404`, turf-vault v0.12.0+).
