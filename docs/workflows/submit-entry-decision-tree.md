...
| unsigned tx on a FRESH blockhash — `Solana::Vault#build_enter_contest` sets no durable nonce | `app/services/solana/vault.rb:2015-2029` |
...
| C1 cosign guard — `Solana::Vault#assert_entry_cosign_safe!` | `#confirm_onchain_entry` at `:1411`; definition `app/services/solana/vault.rb:3026-3122` |
| cosign + simulate + broadcast — `Solana::Vault#cosign_and_broadcast_entry` | `#confirm_onchain_entry` at `app/controllers/contests_controller.rb:1420`; definition `app/services/solana/vault.rb:3346-3366` |
...
2. **Simulation of any tx whose blockhash isn't in the recent queue needs
   ...
   (`app/services/solana/vault.rb:3358-3359`). PR #135.
3. **Never anchor user-driven Phantom-signed txs on a shared durable nonce.**
   ...
   (`app/services/solana/vault.rb:2015-2029`). PR #136.
