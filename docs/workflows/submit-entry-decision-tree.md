...
| unsigned tx on a FRESH blockhash — `Solana::Vault#build_enter_contest` sets no durable nonce | `app/services/solana/vault.rb:1447-1461` |
...
| C1 cosign guard — `Solana::Vault#assert_entry_cosign_safe!` | `#confirm_onchain_entry` at `:1411`; definition `app/services/solana/vault.rb:2323-2419` |
| cosign + simulate + broadcast — `Solana::Vault#cosign_and_broadcast_entry` | `#confirm_onchain_entry` at `app/controllers/contests_controller.rb:1420`; definition `app/services/solana/vault.rb:2643-2663` |
...
1. **Phantom injects Lighthouse guard instructions at signing time, mainnet
   ...
   `Solana::Vault#assert_entry_cosign_safe!`, on its `when lighthouse` arm (`:2402-2406`). PR #134.
2. **Simulation of any tx whose blockhash isn't in the recent queue needs
   ...
   (`app/services/solana/vault.rb:2655-2656`). PR #135.
3. **Never anchor user-driven Phantom-signed txs on a shared durable nonce.**
   ...
   (`app/services/solana/vault.rb:1447-1461`). PR #136.
