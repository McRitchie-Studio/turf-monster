...
| C1 cosign guard — `Solana::Vault#assert_entry_cosign_safe!` | `#confirm_onchain_entry` at `:1411`; definition `app/services/solana/vault.rb:3288-3384` |
| cosign + simulate + broadcast — `Solana::Vault#cosign_and_broadcast_entry` | `#confirm_onchain_entry` at `app/controllers/contests_controller.rb:1420`; definition `app/services/solana/vault.rb:3608-3628` |
...
1. **Phantom injects Lighthouse guard instructions at signing time, mainnet
   ...
   `Solana::Vault#assert_entry_cosign_safe!`, on its `when lighthouse` arm (`:3289-3293`). PR #134.
2. **Simulation of any tx whose blockhash isn't in the recent queue needs
   ...
   (`app/services/solana/vault.rb:3620-3621`). PR #135.
