...
> ~450-line body has a symbol to anchor on. A reader following the SIWS claims is
> therefore the reader most likely to land on the weak branch.
...
3. **Client builds the SIWS message** and calls `provider.signMessage(...)` — `app/views/layouts/application.html.erb:550-551`. The helper prefers the wallet's own `signIn` and falls back to `connect` + `signMessage`; the fallback comment sits at `:525`. The message body begins `wants you to sign in with your Solana account` (`:550`), and the nonce echoed back is checked against `nonceData.nonce` (`:490`).
...
`ContestsController#enter` raises **"No active season configured. Set one at /admin/seasons before users can enter on-chain contests."** when `SeasonConfig.current_season_id.to_i.zero?` (`app/controllers/contests_controller.rb:829-831`), inside its `with_lock` and before any consume. Step 4 fails loudly if step 2b was skipped. Contest *creation* has its own parallel guard: `ContestsController#onchain_season_error` (`:2352-2369`) returns "…before creating on-chain contests." and `#ensure_onchain_season_ready!` (`:2346-2350`) raises it.
...
`Solana::Vault#ensure_user_account` (`app/services/solana/vault.rb:728-737`) is called inline by every entry path — `ContestsController#prepare_entry` calls it at `app/controllers/contests_controller.rb:1020` (step 4). It checks the PDA size and either no-ops, creates the PDA via `Solana::Vault#create_user_account` (`app/services/solana/vault.rb:739-771`), or raises on schema drift. For most admins this is a no-op, because the class-body `after_commit :enqueue_onchain_account_setup, on: :create` (`app/models/user.rb:127`) already ran `User#enqueue_onchain_account_setup` (`:793-795`) at signup, enqueuing `CreateOnchainUserAccountJob` (`app/jobs/create_onchain_user_account_job.rb`; see `docs/AUTH.md`).
...
1. **`GET /contests/new`** — `ContestsController#new` (`app/controllers/contests_controller.rb:83-99`). The form lives at `app/views/contests/new.html.erb`: the `f.collection_select :slate_id` slate picker at `app/views/contests/new.html.erb:84-86`, the `contest[week_span]` select at `:93-95`, `contest_type` at `:123-125`, the hidden `contest_starts_at` **Starts At** field at `:152-153`, and `contest_image` at `:252`. `contest_type` must be one of `Contest.selectable_formats` (`app/models/contest.rb:338-340`; it respects the `ENABLE_TEST_SCAFFOLDING` flag via `AppFlags.test_scaffolding?`). The visible field is **Starts At**, defaulted from the selected slate's first game kickoff by `ContestsController#default_start_for_slate` (`app/controllers/contests_controller.rb:2717-2719`); `Contest#starts_in_at` (`app/models/contest.rb:733-735`) is what the countdown and the on-chain lock timestamp use. `entry_fee_cents` + `max_entries` are derived server-side from `Contest#format_config` (`:349-351`) inside `ContestsController#build_unpersisted_contest_from_params` (`app/controllers/contests_controller.rb:2154-2168`, at `:2157-2158`).
   - **Slate span (slates-sport-year):** the operator may extend a single slate into a **multi-week span** with the `week_span` field. `ContestsController#resolve_span_slate` (`app/controllers/contests_controller.rb:2179-2209`) turns "N weeks from this anchor" into the ONE span slate the contest is played on, building it with `Nfl::BuildSpanSlate.call` if needed (`:2205`). The span is scoped to the anchor slate's season through the **`slates.year` column**, read via `Slate#season_year` (`:2191`), and to its `season_type` (`:2205`) so a preseason anchor cannot yield regular-season weeks. A refusal is captured in `@span_slate_error` (`:2207`) and `ContestsController#create` renders it rather than silently building a shorter contest (`:228-230`). Slate classification reads the **`slates.sport` column** via `Slate#sport`, surfaced by `ContestsController#sport_for_slate` (`:2729-2731`); selectable slates come from `#contest_slate_options` (`:2711-2715`). A span of 1 leaves the plain `slate_id` select untouched (`:2181`).
2. **Submit → `POST /contests`** — `ContestsController#create` (`app/controllers/contests_controller.rb:218-261`):
   - Refuses non-Phantom callers (`:219`).
   - `ContestsController#onchain_create_precheck` (`:2310-2344`, called at `:238`) — model validation (`:2315`), slug uniqueness in the DB via `#slug_taken_message` (`:2325`, definition `:2116-2124`), the on-chain Contest PDA must not already exist (`:2330-2337`), season readiness through `#onchain_season_error` (`:2339`), then `ContestsController#insufficient_usdc_error` (`:2386-2426`, called at `:2343`) verifies the creator's ATA balance covers `guaranteed_prize_cents`.
   - `Solana::Vault#build_create_contest` (`app/services/solana/vault.rb:875-916`, called at `app/controllers/contests_controller.rb:243`) builds a fully UNSIGNED `create_contest` TX with `admin_signs: false` (`:247`) — the admin fee-payer and creator signature slots are both left empty. The account layout is assembled in `Solana::Vault#create_contest_instruction` (`app/services/solana/vault.rb:918-960`): payer, creator, vault_state, contest (init), USDC mint, creator_ata, vault_usdc, token program, system program.
   - The server returns `{ serialized_tx, contest_pda, slug, params_token }` (`app/controllers/contests_controller.rb:250-256`). The `params_token` is a `Rails.application.message_verifier` blob with a 10-minute TTL (`ONCHAIN_CREATE_TOKEN_TTL` — `:216`; signed in `ContestsController#sign_onchain_create_payload`, `:2431-2454`) so the server can trust the re-posted form fields in step 3.4 without re-validating them.
3. **Refresh blockhash + Phantom SIGN (no client broadcast)** — right before signing, the client calls `POST /contests/rebuild_create_tx` (`ContestsController#rebuild_create_tx` — `app/controllers/contests_controller.rb:273-300`, routed by `post :rebuild_create_tx` at `config/routes.rb:300`) to re-issue the unsigned TX over a fresh blockhash; the re-issue goes through the server so the exact message bytes stay bound to the signed `params_token` (`app/controllers/contests_controller.rb:274-287`). The browser then deserializes with `solanaWeb3.Transaction.from` (`app/views/contests/new.html.erb:466`), signs with `provider.signTransaction` (`:480`), and POSTs the signed-but-unbroadcast wire as `signed_tx` to `/contests/finalize` (`:490-503`). There is no `connection.sendRawTransaction` on this path any more: the browser never touches the RPC.
4. **`POST /contests/finalize`** — `ContestsController#finalize` (`app/controllers/contests_controller.rb:332-438`). Collection route (no `:id`), `post :finalize` at `config/routes.rb:301`. In order:
   - `ContestsController#verify_onchain_create_payload` (`app/controllers/contests_controller.rb:2456-2460`) decodes the `params_token` (`:333`) and the user is re-checked against it (`:334`).
   - `Solana::Vault#contest_pda(slug)` (`app/services/solana/vault.rb:181-184`) re-derives the PDA and `finalize` demands `params[:contest_pda]` match (`app/controllers/contests_controller.rb:336-337`).
   - `Solana::Vault#assert_create_contest_cosign_safe!` (`app/services/solana/vault.rb:2311-2394`, called at `app/controllers/contests_controller.rb:353-358`) inspects the signed wire before the server will put its own signature on it.
   - **Write-ahead row.** `ContestsController#build_pending_contest` (`:2251-2289`) constructs the row at `status: :pending` carrying `onchain_contest_id` (`:2263-2265`) and with `skip_onchain_callback = true` (`:2288`) so the legacy `after_create :create_onchain_with_rollback!` hook (`app/models/contest.rb:125`) cannot double-spend. `finalize` saves it at `app/controllers/contests_controller.rb:366`, **before** a lamport moves. `Contest`'s `before_create` binds `season_id` to `SeasonConfig.current_season_id` (`app/models/contest.rb:131`).
   - `Solana::Vault#cosign_and_broadcast_create_contest` (`app/services/solana/vault.rb:2463-2474`) cosigns and broadcasts (`app/controllers/contests_controller.rb:370`). Past this line the money is real.
   - The signature is recorded IMMEDIATELY (`:376`), before any read-back that can raise — the only off-chain evidence tying this request to the on-chain effect.
   - `ContestsController#verify_solana_transaction!` (`:2497-2506`, called at `:395-400`) → `Solana::TxVerifier.verify!` (OPSEC-010) asserts the broadcast TX is the `create_contest` instruction signed by `creator_pubkey` and writing to the derived PDA.
   - Only then is the row promoted to `open` (`:403`), the banner attached last and unable to fail the request (`:411`), and `{ success: true, redirect: contest_path(contest), slug: }` returned (`:413`).
...
1. **Toggle 6 selections** on the matchup board — `POST /contests/:id/toggle_selection` per click (`ContestsController#toggle_selection` — `app/controllers/contests_controller.rb:1424-1445`). Each call `find_or_create_by!`s the cart entry (`:1432`) and toggles a `Selection` row (`:1435`). World Cup Survivor contests use `ContestsController#pick` instead (`:1448-1477`).
...
3. **`POST /contests/:id/prepare_entry`** — `ContestsController#prepare_entry` (`app/controllers/contests_controller.rb:952-1099`):
   - Requires `onchain_session?` (`:975`) — the admin's Phantom-auth session has it from step 1.
   - Requires a verified on-chain contest and a Phantom wallet (`:996-997`), refuses a full contest (`:1000`), validates exactly `picks_required` selections (`:1003` — `Contest#picks_required` at `app/models/contest.rb:267-271`, floored on `TURF_TOTALS_DEFAULT_PICKS_REQUIRED = 6` at `:67`), and refuses if any underlying game is `locked?` (`app/controllers/contests_controller.rb:1005`).
   - Assigns `entry.entry_number` by **probing the chain for a free slot** — `Entry#assign_onchain_entry_number!` (`app/models/entry.rb:307-322`, called at `app/controllers/contests_controller.rb:1015`) reads existing DB entry numbers, then asks `Solana::Vault#next_free_entry_index` (`app/services/solana/vault.rb:205-214`) for the first on-chain-free slot.
   - `Solana::Vault#ensure_user_account(current_user.web3_solana_address, username:)` (`app/controllers/contests_controller.rb:1020`) — see 2d above.
   - `Solana::Vault#build_enter_contest(wallet, slug, entry_num, currency_idx:, season_id:)` (`app/services/solana/vault.rb:1300-1364`, called at `app/controllers/contests_controller.rb:1056`) builds the unified `enter_contest` transaction; a prepared entry token takes `Solana::Vault#build_enter_contest_with_token` instead (`:1039`, definition `app/services/solana/vault.rb:1424-1454`). The Phantom-first flow leaves both signature slots empty — `confirm_onchain_entry` validates the user-signed wire before the server cosigns and broadcasts.
   - Persists a `PendingTransaction` with `tx_type: "enter_contest"`, `status: "pending"` and polymorphic `target: entry` (`app/controllers/contests_controller.rb:1071-1083`), so a mid-flight refresh leaves a recoverable trail.
   - Returns `{ serialized_tx, entry_id, entry_pda, ptx_slug }` (`:1086-1092`).
...
5. **`POST /contests/:id/confirm_onchain_entry`** — `ContestsController#confirm_onchain_entry` (`app/controllers/contests_controller.rb:1284-1402`):
   - It loads the in-flight `PendingTransaction` (`:1317`), then `Solana::Vault#assert_entry_cosign_safe!` (`app/services/solana/vault.rb:2196-2303`, called at `app/controllers/contests_controller.rb:1331`) refuses to cosign a wire that is not exactly one `enter_contest` instruction binding the session's wallet to the derived entry PDA.
   - `Solana::Vault#cosign_and_broadcast_entry` (`app/services/solana/vault.rb:2400-2420`) cosigns and broadcasts (`app/controllers/contests_controller.rb:1340`); the signature is stamped onto the `PendingTransaction` immediately (`:1352`), because it is on-chain whether or not the steps below raise.
   - `ContestsController#verify_and_confirm_onchain_entry!` (`:2554-2570`, called at `:1358-1361` with the client-supplied `params[:entry_pda]`) re-derives `entry_pda` via `Solana::Vault#entry_pda(slug, wallet, entry_number)` (`app/controllers/contests_controller.rb:2556-2558`, definition `app/services/solana/vault.rb:186-191`) and rejects a mismatched client-supplied PDA (`app/controllers/contests_controller.rb:2559`).
   - `#verify_and_confirm_onchain_entry!` then calls `verify_solana_transaction!` (`:2561-2566`), asserting the TX is `enter_contest` signed by the user's wallet and writing to the derived entry PDA (OPSEC-010).
   ...
   - The `PendingTransaction` is marked `confirmed` (`app/controllers/contests_controller.rb:1363`) and the response carries `{ success: true, redirect, tx_signature, seeds_earned, seeds_total, seeds_level }` (`:1380-1387`).
...
- **DB:**
  ...
  - `slates` (read — the selected slate; on a span, its consecutive weekly siblings, scoped by the `year`/`sport`/`season_type` columns in `ContestsController#resolve_span_slate` → `Nfl::BuildSpanSlate` — `app/controllers/contests_controller.rb:2205`)
  - `slate_matchups` (read — the pickable matchups behind the selections)
  - `contests` (insert at `status: :pending` in `ContestsController#finalize` — `app/controllers/contests_controller.rb:366` — then `onchain_tx_signature` at `:376` and promotion to `open` at `:403`)
  - `entries` (insert via `ContestsController#toggle_selection` at `:1432`; update to `active` via `Entry#confirm_onchain!` at `app/models/entry.rb:242-272`)
  - `selections` (insert per matchup toggle, inside `Entry#toggle_selection!` — `app/models/entry.rb:54`)
  - `pending_transactions` (insert in `ContestsController#prepare_entry` at `app/controllers/contests_controller.rb:1071`; `tx_signature` + `status` updated through the lifecycle by `ContestsController#confirm_onchain_entry` at `:1352` and `:1363`)
...
- [[email-signup-token-to-chat]] — the managed-wallet alternative: `ContestsController#enter` falls through to `#resolve_web2_entry_funding!` (`app/controllers/contests_controller.rb:1821-1893`), which consumes an `EntryTokenAccount` PDA through `Solana::Vault#enter_contest_with_token` (`:1841`) instead of charging USDC. An admin never hits this branch — `#enter` routes it only for a non-`onchain_session?` request (`:842`).
