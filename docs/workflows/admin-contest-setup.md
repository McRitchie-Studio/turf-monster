...
3. **Client builds the SIWS message** and calls `provider.signMessage(...)` — `app/views/layouts/application.html.erb:562-563`. The helper prefers the wallet's own `signIn` and falls back to `connect` + `signMessage`, announcing the fallback with a `console.warn` at `:537`. The message body begins `wants you to sign in with your Solana account` (`:562`), and the nonce echoed back is checked against `nonceData.nonce` (`:502`).
