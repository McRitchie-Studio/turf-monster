...
- The on-chain `Season` PDA lives at `[b"season", season_id_le]` — derived by `Solana::Vault#season_pda` (`app/services/solana/vault.rb:549-552`) — and stores the `seed_schedule` (default `[25, 19, 14, 10, 7]`) the `enter_contest` instruction reads to award seeds (see `docs/SOLANA.md`).

#### 2c. Per-contest Contest PDA init — **fires every time** in step 3

The contest PDA at `[b"contest", sha256(slug)]`, derived by `Solana::Vault#contest_pda` (`app/services/solana/vault.rb:471-474`), is created by the `create_contest` instruction in step 3 below. There is no separate "init contract" click for this.
...
2. **Submit → `POST /contests`** — `ContestsController#create` (`app/controllers/contests_controller.rb:267-310`):
   ...
   - `Solana::Vault#build_create_contest` (`app/services/solana/vault.rb:1693-1734`, called at `app/controllers/contests_controller.rb:292`) builds a fully UNSIGNED `create_contest` TX with `admin_signs: false` (`:296`) — the admin fee-payer and creator signature slots are both left empty. The account layout is assembled in `Solana::Vault#create_contest_instruction` (`app/services/solana/vault.rb:1736-1779`): payer, creator, vault_state, contest (init), USDC mint, creator_ata, vault_usdc, token program, system program.
...
4. **`POST /contests/finalize`** — `ContestsController#finalize` (`app/controllers/contests_controller.rb:381-487`). Collection route (no `:id`), `post :finalize` at `config/routes.rb:301`. In order:
   ...
   - `Solana::Vault#contest_pda(slug)` (`app/services/solana/vault.rb:471-474`) re-derives the PDA and `finalize` demands `params[:contest_pda]` match (`app/controllers/contests_controller.rb:385-386`).
   - `Solana::Vault#assert_create_contest_cosign_safe!` (`app/services/solana/vault.rb:3460-3539`, called at `app/controllers/contests_controller.rb:402-407`) inspects the signed wire before the server will put its own signature on it.
   ...
   - `Solana::Vault#cosign_and_broadcast_create_contest` (`app/services/solana/vault.rb:3816-3822`) cosigns and broadcasts (`app/controllers/contests_controller.rb:419`). Past this line the money is real.
...
3. **`POST /contests/:id/prepare_entry`** — `ContestsController#prepare_entry` (`app/controllers/contests_controller.rb:1024-1179`):
   ...
   - Assigns `entry.entry_number` by **probing the chain for a free slot** — `Entry#assign_onchain_entry_number!` (`app/models/entry.rb:307-322`, called at `app/controllers/contests_controller.rb:1087`) reads existing DB entry numbers, then asks `Solana::Vault#next_free_entry_index` (`app/services/solana/vault.rb:495-504`) for the first on-chain-free slot.
   ...
   - `Solana::Vault#build_enter_contest(wallet, slug, entry_num, currency_idx:, season_id:)` (`app/services/solana/vault.rb:2186-2272`, called at `app/controllers/contests_controller.rb:1128`) builds the unified `enter_contest` transaction; a prepared entry token takes `Solana::Vault#build_enter_contest_with_token` instead (`:1111`, definition `app/services/solana/vault.rb:2333-2364`). The Phantom-first flow leaves both signature slots empty — `confirm_onchain_entry` validates the user-signed wire before the server cosigns and broadcasts.
...
5. **`POST /contests/:id/confirm_onchain_entry`** — `ContestsController#confirm_onchain_entry` (`app/controllers/contests_controller.rb:1364-1482`):
   - It loads the in-flight `PendingTransaction` (`:1397`), then `Solana::Vault#assert_entry_cosign_safe!` (`app/services/solana/vault.rb:3353-3451`, called at `app/controllers/contests_controller.rb:1411`) refuses to cosign a wire that is not exactly one `enter_contest` instruction binding the session's wallet to the derived entry PDA.
   - `Solana::Vault#cosign_and_broadcast_entry` (`app/services/solana/vault.rb:3676-3691`) cosigns and broadcasts (`app/controllers/contests_controller.rb:1420`); the signature is stamped onto the `PendingTransaction` immediately (`:1432`), because it is on-chain whether or not the steps below raise.
   - `ContestsController#verify_and_confirm_onchain_entry!` (`:2722-2738`, called at `:1438-1441` with the client-supplied `params[:entry_pda]`) re-derives `entry_pda` via `Solana::Vault#entry_pda(slug, wallet, entry_number)` (`app/controllers/contests_controller.rb:2724-2726`, definition `app/services/solana/vault.rb:476-481`) and rejects a mismatched client-supplied PDA (`app/controllers/contests_controller.rb:2727`).
...
- **On-chain (turf-vault):**
  - `VaultState` PDA at `[b"vault"]` — `Solana::Vault#vault_state_pda` (`app/services/solana/vault.rb:263-265`); read, and **init** if 2a fires
  - `Season` PDA at `[b"season", season_id_le]` — `Solana::Vault#season_pda` (`:549-552`); read, and **init** if 2b fires
  - `UserAccount` PDA at `[b"user", wallet]` — `Solana::Vault#user_account_pda` (`:466-469`); read, and **init** if 2d fires
  - `Contest` PDA at `[b"contest", sha256(slug)]` — `Solana::Vault#contest_pda` (`:471-474`); **init** via the `create_contest` IX in step 3
  - `ContestEntry` PDA at `[b"entry", contest_pda, wallet, entry_num_le]` — `Solana::Vault#entry_pda` (`:476-481`); **init** via the `enter_contest` IX in step 4
