...
- Admin UI: `Admin::SeasonsController#create` (`app/controllers/admin/seasons_controller.rb:11-39`) reads `name`, `season_id`, and `slot_0..slot_4` from the form (`:14`), validates the five slots (`:22`), calls `Solana::Vault#create_season(season_id:, name:, schedule:)` (`:31`, definition `app/services/solana/vault.rb:2753-2800`), and — when `params[:set_current] == "1"` — flips `SeasonConfig.set_current!(season_id)` (`app/controllers/admin/seasons_controller.rb:32`).
...
4. **`POST /contests/finalize`** — `ContestsController#finalize` (`app/controllers/contests_controller.rb:381-487`). Collection route (no `:id`), `post :finalize` at `config/routes.rb:301`. In order:
   ...
   - `Solana::Vault#assert_create_contest_cosign_safe!` (`app/services/solana/vault.rb:3130-3206`, called at `app/controllers/contests_controller.rb:402-407`) inspects the signed wire before the server will put its own signature on it.
   ...
   - `Solana::Vault#cosign_and_broadcast_create_contest` (`app/services/solana/vault.rb:3409-3420`) cosigns and broadcasts (`app/controllers/contests_controller.rb:419`). Past this line the money is real.
...
3. **`POST /contests/:id/prepare_entry`** — `ContestsController#prepare_entry` (`app/controllers/contests_controller.rb:1024-1179`):
   ...
   - `Solana::Vault#build_enter_contest(wallet, slug, entry_num, currency_idx:, season_id:)` (`app/services/solana/vault.rb:1984-2048`, called at `app/controllers/contests_controller.rb:1128`) builds the unified `enter_contest` transaction; a prepared entry token takes `Solana::Vault#build_enter_contest_with_token` instead (`:1111`, definition `app/services/solana/vault.rb:2109-2140`). The Phantom-first flow leaves both signature slots empty — `confirm_onchain_entry` validates the user-signed wire before the server cosigns and broadcasts.
...
5. **`POST /contests/:id/confirm_onchain_entry`** — `ContestsController#confirm_onchain_entry` (`app/controllers/contests_controller.rb:1364-1482`):
   - It loads the in-flight `PendingTransaction` (`:1397`), then `Solana::Vault#assert_entry_cosign_safe!` (`app/services/solana/vault.rb:3026-3122`, called at `app/controllers/contests_controller.rb:1411`) refuses to cosign a wire that is not exactly one `enter_contest` instruction binding the session's wallet to the derived entry PDA.
   - `Solana::Vault#cosign_and_broadcast_entry` (`app/services/solana/vault.rb:3346-3366`) cosigns and broadcasts (`app/controllers/contests_controller.rb:1420`); the signature is stamped onto the `PendingTransaction` immediately (`:1432`), because it is on-chain whether or not the steps below raise.
...
- **IDL hash drift after a turf-vault upgrade** — `Solana::Config.verify_idl!` refuses to boot and to precompile in production (`docs/SOLANA.md:460`). Borsh decoding would silently corrupt every account read otherwise. The operator must re-pin `EXPECTED_IDL_HASH` from the freshly **built** IDL — NOT `anchor idl fetch`, which returns the stale pre-upgrade IDL because a Squad upgrade runs only the BPF `upgrade` instruction (`docs/SOLANA.md:439`) — before pushing. See the `feedback_post_deploy_idl_pin` memory.
