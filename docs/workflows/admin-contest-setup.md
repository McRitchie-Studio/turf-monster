...
2. **Submit → `POST /contests`** — `ContestsController#create` (`app/controllers/contests_controller.rb:218-261`):
   - Refuses non-Phantom callers (`app/controllers/contests_controller.rb:219`).
...
5. **`POST /contests/:id/confirm_onchain_entry`** — `ContestsController#confirm_onchain_entry` (`app/controllers/contests_controller.rb:899-994`):
   - Delegates to `verify_and_confirm_onchain_entry!` (`app/controllers/contests_controller.rb:2554-2570`, called at `:1358-1361` with the client-supplied `params[:entry_pda]`), which re-derives `entry_pda` via `Solana::Vault#entry_pda(slug, wallet, entry_number)` (`app/controllers/contests_controller.rb:2556-2558`) and rejects a mismatched client-supplied PDA (`:2559`).
   - `verify_solana_transaction!` (`app/controllers/contests_controller.rb:2561-2566`) asserts the TX is `enter_contest` signed by the user's wallet, cosigned by the admin server key, and writing to the derived entry PDA (OPSEC-010).
...
- **Insufficient USDC for prize pool** — `onchain_create_precheck` calls `insufficient_usdc_error` (`app/controllers/contests_controller.rb:2343`, defined at `:2386-2426`); client modal offers a "Mint $500 Test USDC" recovery button (`app/views/contests/new.html.erb:366`) hitting `POST /faucet` from `mintTestUsdcAndRetry` (`app/views/contests/new.html.erb:373`). `FaucetController#claim` is production-disabled per OPSEC-020 (`app/controllers/faucet_controller.rb:32`).
