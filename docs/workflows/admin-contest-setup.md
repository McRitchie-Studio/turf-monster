...
4. **`POST /contests/finalize`** — `ContestsController#finalize` (`app/controllers/contests_controller.rb:381-487`). Collection route (no `:id`), `post :finalize` at `config/routes.rb:301`. In order:
   ...
   - `Solana::Vault#assert_create_contest_cosign_safe!` (`app/services/solana/vault.rb:3456-3534`, called at `app/controllers/contests_controller.rb:402-407`) inspects the signed wire before the server will put its own signature on it.
...
5. **`POST /contests/:id/confirm_onchain_entry`** — `ContestsController#confirm_onchain_entry` (`app/controllers/contests_controller.rb:1364-1482`):
   - It loads the in-flight `PendingTransaction` (`:1397`), then `Solana::Vault#assert_entry_cosign_safe!` (`app/services/solana/vault.rb:3351-3448`, called at `app/controllers/contests_controller.rb:1411`) refuses to cosign a wire that is not exactly one `enter_contest` instruction binding the session's wallet to the derived entry PDA.
