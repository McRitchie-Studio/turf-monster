...
- **IDL hash drift after a turf-vault upgrade** — `Solana::Config.verify_idl!` refuses to boot and to precompile in production (`docs/SOLANA.md:621`). Borsh decoding would silently corrupt every account read otherwise. The operator must re-pin `EXPECTED_IDL_HASH` from the freshly **built** IDL — NOT `anchor idl fetch`, which returns the stale pre-upgrade IDL because a Squad upgrade runs only the BPF `upgrade` instruction (`docs/SOLANA.md:592`) — before pushing. See the `feedback_post_deploy_idl_pin` memory.
