...
4. **`POST /contests/finalize`** — `ContestsController#finalize` (`app/controllers/contests_controller.rb:381-487`). Collection route (no `:id`), `post :finalize` at `config/routes.rb:301`. In order:
   ...
   - `Solana::Vault#cosign_and_broadcast_create_contest` (`app/services/solana/vault.rb:2463-2474`) cosigns and broadcasts (`app/controllers/contests_controller.rb:419`). Past this line the money is real.
...
5. **`POST /contests/:id/confirm_onchain_entry`** — `ContestsController#confirm_onchain_entry` (`app/controllers/contests_controller.rb:1341-1459`):
   - It loads the in-flight `PendingTransaction` (`:1374`), then `Solana::Vault#assert_entry_cosign_safe!` (`app/services/solana/vault.rb:2196-2303`, called at `app/controllers/contests_controller.rb:1388`) refuses to cosign a wire that is not exactly one `enter_contest` instruction binding the session's wallet to the derived entry PDA.
