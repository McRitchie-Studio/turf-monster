...
| Focus-game priority list — `resources :weeks` | `config/routes.rb:500-502` — admin only |
...
(`db/schema.rb:321`) is a position in ONE list covering the whole week — unique per
season slot (year + season type + week) through the partial index
`index_games_on_focus_rank_per_slot` (`db/schema.rb:340`), and validated as a positive
...
| Drag | Request | Effect |
|---|---|---|
| re-sorts the list | `POST /admin/nfl/weeks/:slot/reorder` — `member { post :reorder }` (`config/routes.rb:501`) | `Admin::Nfl::WeeksController#reorder` (`app/controllers/admin/nfl/weeks_controller.rb:53-67`) makes the list's order `focus_rank` 1..n |
...
- **It will not store an id-less play.** `play["id"].to_s` yields `""`, which the
  ...
  `WHERE external_id IS NOT NULL` predicate (`db/schema.rb:361`) — so a second id-less
