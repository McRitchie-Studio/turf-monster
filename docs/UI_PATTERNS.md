...
**Quest 2 composer nudge**: While the "Send Your First Message" mission is showing (`contests/_quest_chat`), the composer answers the quest card's southwest arrow with two cues, both driven by the one `quest-chat-active` window event `questCard#_maybePingChat` dispatches. `contestChat#questChatActive()` pops the panel, sets `questGlow` (binds `.chat-input-glow` — a border + box-shadow pulse on the `<textarea>` itself, since a form control carries no `::before`/`::after` for the `.studio-border-glow` primitive), and starts the placeholder typewriter. The deck comes from `ContestsHelper#chat_prompt_samples`, rendered per-viewer into `data-chat-prompts`, and is SEMI-STATIC: two fixed openers (`Hey everyone 👋`, `Good luck, everyone ⚔️`) then one personal line naming the viewer's longest-priced pick (`"<mascot> are about to light it up <team emoji>"`, highest `turf_score` — the curve pins rank 1 at x1.0, so the biggest multiplier is the biggest swing; falls back to the contest's own longest price, then to `CHAT_PROMPT_NO_TEAM`). The deck size IS the pass count (`CHAT_PROMPT_LIMIT`, 3): the composer types each line in turn and then RESTS on the last one instead of looping, so the personal line is what stays in the placeholder — a prompt that rewrites itself forever under the caret stops being a prompt. Typing steps over CODE POINTS, never string indexes — slicing mid-emoji paints a lone surrogate. Both cues retire on the first message that earns its seeds (`questDone()`), on any keystroke (`$watch('body')`), and under `prefers-reduced-motion` (static placeholder, halo without the pulse).
