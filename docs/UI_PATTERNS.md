...
**The `micro` tier is break-even by design** (operator call, 2026-08-27): a full contest grosses exactly the $9 it guarantees, and a short fill loses more than it takes in (1 entry = $1 in, $9 out). It exists to rehearse the money path, not to earn on it. `test/models/contest_test.rb` pins the $0 margin so a later "rounding" edit has to be deliberate.
