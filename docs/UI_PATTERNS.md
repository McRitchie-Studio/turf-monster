...
None of those three is visible in the response bytes, so they are guarded in a browser by `e2e/entry_badge_sidebar.spec.js`: it asserts the shadow by neutralising `--lb-contact` and watching the PIXELS move (freezing the gradient pan with `emulateMedia` first, since `playwright.config`'s `reducedMotion` does not reach the page), the panel still open a beat after the click, and the chip across both mounts.
