...
- **Levels**: `fizz_level: :calm` (default) simmers at rest and boils on hover. `fizz_level: :lively` adds `.fizz-lively` to the stack — it rests AT the calm hover, and hovering doubles the bubble COUNT at the same speed, for a button carrying a page on its own. Held/submitting/confirmed/blocked are identical in both.
