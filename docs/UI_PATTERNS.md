...
way in. The admin preview seam is not a way in either:
`AdminController#modal_preview` passes `params[:modal_id]` through raw to
`$store.modals.open()`, but `layouts/modal_preview` never registers or renders
the partial — so a hand-crafted `/admin/modals/preview/wallet-topup` opens the
host over a panel that is not in the DOM, giving an empty card rather than an
entrance. That layout's registration list is what decides, and always was: this
paragraph used to rest the same conclusion on `AdminController::MODAL_VARIANTS`
having no `wallet-topup` row, and the constant was deleted with the gallery on
2026-09-08 without the conclusion changing at all.
