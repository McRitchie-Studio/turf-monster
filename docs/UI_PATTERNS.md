...
- **Danger text needs a theme surface under it.** `text-danger-ink` is derived to clear AA against the four theme surfaces (`bg-surface`, `bg-page`, `bg-surface-alt`, `bg-inset`), and a tint is not one of them — compositing `bg-red-500/10` over the dark card yields `#673751`, where the ink measures **3.78:1** and fails AA. Put an alert panel on a theme surface (`bg-inset` or `bg-surface-alt`; the ink is 7.31:1 on the dark `bg-surface-alt`) and keep the red *border* for the affordance, rather than a red wash. The guard composites every enclosing background before measuring, so a red-tinted panel is caught rather than scored against the bare card.
