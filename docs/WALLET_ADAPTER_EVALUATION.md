...
| **2. Session replay stays suppressed on every page a sensitive hop lands on.** Only `WalletExportsController#harden_secret_response` suppresses it (`app/controllers/wallet_exports_controller.rb:101`); `session_replay_active?` is otherwise true in production (`app/helpers/application_helper.rb:147-149`). | ✅ for transactions. The callback runs with replay **on**, which is why export must never cross it. | ✅ no landing page | ⚠ its auth redirect lands on a page we choose; the page must suppress replay if it carries anything sensitive | ✅ for WalletConnect; the in-app browser path is our inline path |
