...
gem "solana-studio", "~> 0.9", ">= 0.9.2" # 0.9.2 is the real floor, and the pin SAYS so — and like the engine pin below, this floor is a PATCH, which a two-segment `~>` CANNOT state, so the line carries two requirements instead of one. THE INLINE TRANSACTION CONTRACT is what moved it. This app now runs contest entry, contest create and bundle provisioning through ONE walletOps.run per call site on BOTH transports, and that is only possible because 0.9.2 taught the INLINE path to take base58 wire bytes and convert them through the provider's own deserializeTransaction / serializeTransaction (app/javascript/wallet_provider.js). Below 0.9.2 runInline hands `prepared.transaction` — a base58 STRING — straight to an injected wallet's signTransaction, which throws "t.serialize is not a function" from inside the extension's own code on EVERY desktop signature in all three flows: loud, but from a stack trace that names nothing in this app. `expectedAccount` arrived in the same release and is the SILENT half, which is why this note is worth its length: below 0.9.2 the option is simply not read, so the wrong-wallet guard every one of those call sites now declares vanishes with nothing raised, logged, or failed — the operator signs with the wrong account and learns about it from an Anchor error. DERIVED TWO WAYS, neither of them a changelog. (a) Unpacking the published gems: 0.8.0, 0.9.0 and 0.9.1 contain ZERO occurrences of requireInlineCodec and of expectedAccount, and 0.9.2 contains both. (b) In the gem repo, the EARLIEST tag containing the commit that adds them (33b2f75, "Unify walletOps' two transports on one transaction contract") is v0.9.2. Stated as the EARLIEST containing version for the reason the notes below record at length — a LIST of versions goes stale on the next release, while "earliest" is a fact about the code, and test/lib/engine_pin_contract_test.rb re-derives the comparison on every run rather than trusting this sentence. VISIBLE TO THE RESOLVER, unlike most floors in this file: the old "~> 0.9" admitted 0.9.0 and 0.9.1, which is precisely the window that breaks. Gemfile.lock already resolves 0.9.2, so no install changes today; the exposure was a lock rebuilt from this file alone. PRIOR FLOOR NOTE, still true: 0.9.0 is the real floor, and the pin SAYS so. This app now LOADS the redirect transport (solana_studio/wallet_transport, redirect_provider, wallet_journal, wallet_ops — see layouts/application) and registers the contest_entry intent against it, which is what gives a phone any way to sign at all. TWO SEPARATE ARRIVALS, and the floor is the LATER one. (1) The transport itself first exists in 0.8.0 — below that `SolanaStudio.walletOps` is undefined and walletOps.run throws on the first hold-to-confirm, a LOUD failure. (2) The floor that actually moved the pin is 0.9.0, and it fails SILENTLY, which is the only reason this note is worth its length: solana_studio/modals/_wallet_connect gained mobileHandoffs + openInWallet there, the getters that paint a Solflare or Backpack row on a phone. Below 0.9.0 the picker never asks the registry this app now loads, so those two wallets fall back to their DESKTOP EXTENSION download rows — the exact dead end /tasks/fix-wallet-picker-deeplink-claim removed — and nothing raises, logs, or fails a test. DERIVED, not read off a changelog: unpacking the published gems, 0.8.0 ships the four transport files with ZERO mobileHandoffs in the picker, and 0.9.0 ships both. INVISIBLE TO THE RESOLVER in the usual way — the old "~> 0.6" already admitted 0.9.0 and the lockfile resolved 0.7.0 — so this bump is the FLOOR moving, and `bundle update solana-studio` is what makes the lock agree.
