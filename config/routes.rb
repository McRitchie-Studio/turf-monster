...
Rails.application.routes.draw do
  ...
  scope :cdp do
    ...
    # Offramp send path (§10) — managed confirm-then-server-send, and the Phantom
    # prepare/sign/cosign/report loop, keyed on partner_user_ref and scoped to the
    # viewer's own rows. format: false — rack-attack matches these paths EXACTLY.
    post "offramp/confirm_send", to: "cdp/offramp_sends#confirm", as: :cdp_offramp_confirm_send
    post "offramp/prepare_send", to: "cdp/offramp_sends#prepare", as: :cdp_offramp_prepare_send, format: false
    post "offramp/cosign_send",  to: "cdp/offramp_sends#cosign", as: :cdp_offramp_cosign_send, format: false
